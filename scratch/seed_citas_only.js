...
async function run() {
    console.log("Fetching new pacientes...");
    const { data: newPacientes, error: errPac } = await supabase.from('pacientes').select('*').limit(15).order('created_at', { ascending: false });

    if (errPac || !newPacientes) {
        console.error("Error fetching pacientes:", errPac);
        return;
    }
    ...
    for (let i = 0; i < 30; i++) {
        ...
        citasToInsert.push({
            ...
            estado: estado,
            // doctor: 'Dr. Alejandro Eslava', // removed
